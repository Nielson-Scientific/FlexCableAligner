...
class FlexAlignerGUI:
    ...
    def _handle_buttons(self):
        ...
        if self.joystick.get_button(0):
            if not hasattr(self, '_last_fine') or t - self._last_fine > 0.5:
        ...
        if self.joystick.get_button(1):
            if not hasattr(self, '_last_save') or t - self._last_save > 0.5:
        ...
        if self.joystick.get_button(3) and self.selected_row_index is not None and self.selected_row_index < len(self.positions_list):
            if not hasattr(self, '_last_goto') or t - self._last_goto > 0.5:
                self.goto_saved_position()
                self._last_goto = t
        # 2: home XY
        if self.joystick.get_button(2):
            if not hasattr(self, '_last_home') or t - self._last_home > 1.0:
